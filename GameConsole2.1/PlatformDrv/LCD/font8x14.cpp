...
static const unsigned char table8x14[3136] PROGMEM = {
...
void put_char8x14_8(const coord x, const coord y, const char c, const color8 color, const color8 bgcolor) {
	register unsigned char mask, cl, ch;
	int base = reinterpret_cast<int>(table8x14) + 14 * (c - 0x20);
	SetWindow(x, y, 8, 14);
	SetDatMode();
	for (cl = 0; cl<14; cl++) {
		ch = pgm_read_byte(base + cl);
...
void put_char8x14_16(const coord x, const coord y, const char c, const color16 color, const color16 bgcolor) {
	register unsigned char mask, cl, ch;
	int base = reinterpret_cast<int>(table8x14) + 14 * (c - 0x20);
	SetWindow(x, y, 8, 14);
	SetDatMode();
...
void put_char8x14_8BGL(const coord x, const coord y, const char c, const color8 color) {
	register unsigned char mask, cl, ch, off_x = 0, off_y = 0;
	int base = reinterpret_cast<int>(table8x14) + 14 * (c - 0x20);
	for (cl = 0; cl<14; cl++, off_y++) {
		ch = pgm_read_byte(base + cl);
		for (mask = 0x80, off_x = 0; mask; mask>>=1, off_x++) {
			if (ch & mask) {
				SetPixel(x + off_x, y + off_y, color);
			}
		}
	}
}

void put_char8x14_16BGL(const coord x, const coord y, const char c, const color16 color) {
	register unsigned char mask, cl, ch, off_x = 0, off_y = 0;
	int base = reinterpret_cast<int>(table8x14) + 14 * (c - 0x20);
	for (cl = 0; cl<14; cl++, off_y++) {
		ch = pgm_read_byte(base + cl);
